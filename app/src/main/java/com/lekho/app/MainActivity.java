...
public class MainActivity extends AppCompatActivity {
            ...
            @Override public void onError(int error) {
                ...
                switch (error) {
                    ...
                    case SpeechRecognizer.ERROR_RECOGNIZER_BUSY:
                        msg = "Mic अभी busy है। 1 सेकंड बाद फिर दबाएँ।"; break;
                    default:
    ...
    private void sendResult(String text) {
        ...
        webView.post(() -> webView.evaluateJavascript(
                "if(window.__nativeSpeechResult){window.__nativeSpeechResult('" + safe + "')} else if(window.__lekhoSpeechCallback){window.__lekhoSpeechCallback('" + safe + "')}", null));
    }
    ...
    private void sendError(String message) {
        ...
        webView.post(() -> webView.evaluateJavascript(
                "if(window.__nativeSpeechError){window.__nativeSpeechError('" + safe + "')} else if(window.showMicStatus){window.showMicStatus('" + safe + "')}", null));
    }
    ...
    private void sendStatus(String message) {
        ...
        String safe = jsSafe(message);
        webView.post(() -> webView.evaluateJavascript(
                "window.showMicStatus && window.showMicStatus('" + safe + "')", null));
    }
                ...
                try {
                    ...
                    PhoneAuthOptions options = PhoneAuthOptions.newBuilder(firebaseAuth)
                            ...
                            .setCallbacks(new PhoneAuthProvider.OnVerificationStateChangedCallbacks() {
                                ...
                                @Override public void onVerificationFailed(@NonNull FirebaseException e) { sendOtpError("OTP नहीं भेजा जा सका। " + firebaseErrorMessage(e)); }
            ...
            runOnUiThread(() -> {
                try {
                    ...
                    firebaseAuth.signInWithCredential(credential).addOnCompleteListener(task -> {
                        ...
                        } else sendOtpError("OTP गलत है या expire हो गया।");
                    });
                } catch (Exception e) {
    ...
    private void sendOtpSent(String id) { if (webView != null) { String s=jsSafe(id); webView.post(() -> webView.evaluateJavascript("window.__otpSent&&window.__otpSent('"+s+"')", null)); } }
    private void sendOtpVerified() { if (webView != null) webView.post(() -> webView.evaluateJavascript("window.__otpVerified&&window.__otpVerified()", null)); }
    private void sendOtpError(String m) { if (webView != null) { String s=jsSafe(m); webView.post(() -> webView.evaluateJavascript("window.__otpError&&window.__otpError('"+s+"')", null)); } }
    ...
    private String firebaseErrorMessage(Exception e) {
        ...
        if(x.contains("app-not-authorized")) return "Firebase Phone Auth authorization/SHA-1 जांचें।";
    ...
    public void onRequestPermissionsResult(int requestCode,@NonNull String[] permissions,@NonNull int[] grantResults) {
        ...
        if(requestCode==MIC_REQUEST){
            ...
            if(granted && start) startNativeSpeech();
            else if(!granted) sendError("Microphone permission Allow करें, तभी Voice Typing चलेगी।");
        }
    }

    @Override public void onBackPressed(){
        if(webView==null){super.onBackPressed();return;}
        webView.evaluateJavascript("(function(){var pv=document.getElementById('lekhoPrintPreview');if(pv){pv.remove();return 'handled'}var m=document.querySelector('.productModal');if(m){m.remove();return 'handled'}if(window.__nativeBack&&window.__nativeBack())return 'handled';return 'exit'})()",v->{if("\"exit\"".equals(v)) MainActivity.super.onBackPressed();});
    }
